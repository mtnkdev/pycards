...
from pysollib.gametype import CATEGORY, STYLE
...
##            hint = Struct(),            #   extra info for class Hint
    ...
    def _checkGame(self):
        ...
        if self.s.foundations:
            ...
            if ncards != self.gameinfo.numcards:
                print_err('invalid sum of foundations.max_cards: '
                          '%s: %s %s' % (class_name, ncards, self.gameinfo.numcards),
                          2)
        if self.s.rows:
    ...
    def newGame(self, random=None, restart=0, autoplay=1):
        ...
        self.startGame()
        if self.gameinfo.game_flags & STYLE.OPEN:
            if self.s.talon:
    ...
    def createCards(self, progress=None):
        ...
        pstep = 0
        if progress:
            pstep = (100.0 - progress.percent) / gi.numcards
        cards = []
        id = 0
    ...
    def shuffleSeparateDecks(self):
        cards = []
        self.random.reset()
        n = self.gameinfo.numcards / self.gameinfo.decks
...
in the current implementation.''') % version)
        ...
        validate(game.loadinfo.ncards == game.gameinfo.numcards, err_txt)
    ...
    def canFindCard(self):
        return self.gameinfo.category == CATEGORY.FRENCH
