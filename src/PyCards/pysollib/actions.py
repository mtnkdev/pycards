...
from pysollib.gametype import STYLE, SKILL
...
class PysolMenubar(PysolMenubarTk):
    ...
    def _updateMenuState(self):
        ...
        if not game.finished:
            ms.pause = 1
        if game.gameinfo.game_type == STYLE.CUSTOM:
            ms.custom_game = 1
        if game.canShuffle():
            if opt.shuffle:
                ms.shuffle = 1
    ...
    def mSelectRandomGame(self, type='all'):
        ...
        game_id = None
        games = []
        for g in self.app.gdb.gamesIdSorted():
    ...
    def mSelectNextGameById(self, *args):
        self._mSelectNextGameFromList(self.app.gdb.gamesIdSorted(), 1)

    def mSelectPrevGameById(self, *args):
        self._mSelectNextGameFromList(self.app.gdb.gamesIdSorted(), -1)

    def mSelectNextGameByName(self, *args):
        self._mSelectNextGameFromList(self.app.gdb.gamesNameSorted(), 1)
