...
from gamedb import GAME_DB, loadGame
from pysollib.gametype import CATEGORY
    ...
    def mainloop(self):
        ...
        if not self.nextgame.loadedgame:
            if self.commandline.loadgame:
                ...
                gameid = self.gdb.getGame(self.commandline.game)
                if gameid is None:
...
''') % (CARDSET, self.cardset.name, gi.name, t[0], CARDSET),
    ...
    def getGamesIdSortedById(self):
        return self.gdb.gamesIdSorted()

    def getGamesIdSortedByName(self, player=''):
        return self.gdb.gamesNameSorted()
    ...
    def getGamesIdSortedByPlayed(self, player=''):
        ...
        games = list(self.gdb.gamesNameSorted())
        games.sort(_cmp)
        return games
...
##             n = m.group(1) + "_" + m.group(2).lower()
    ...
    def getRandomGameId(self, games=None):
        if games is None:
            return self.miscrandom.choice(self.gdb.gamesIdSorted())
