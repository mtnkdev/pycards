...
class SelectGameDialogWithPreview(MfxDialog):
    ...
    def createGameStore(self):
        ...
        all_games = map(gdb.get, gdb.gamesNameSorted())
        #
        alter_games = gdb.gamesAltNameSorted()
        ...
        self._addGamesFromData(GI.SELECT_FR_GAME_BY_TYPE, store,
                               None, _("French games"), all_games)
        # by skill level
        data = (
          (_('Luck only'),    lambda gi: gi.skill_level == SKILL.LUCK),
          (_('Mostly luck'),  lambda gi: gi.skill_level == SKILL.MOSTLY_LUCK),
          (_('Balanced'),     lambda gi: gi.skill_level == SKILL.BALANCED),
          (_('Mostly skill'), lambda gi: gi.skill_level == SKILL.MOSTLY_SKILL),
          (_('Skill only'),   lambda gi: gi.skill_level == SKILL.SKILL),
          )
    ...
    def updateInfo(self, gameid):
        ...
        type = ''
        if gi.si.game_type in GI.FR_TYPES:
            type = _(GI.FR_TYPES[gi.si.game_type])
        sl = {
