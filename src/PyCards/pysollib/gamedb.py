...
class GI:
    ...
    #     # ALL DIFFERENT GAME TYPES UNDER ALL CATEGORIES
    # GT_1DECK_TYPE   = 0
    # GT_2DECK_TYPE   = 1
    # GT_3DECK_TYPE   = 2
    # GT_4DECK_TYPE   = 3
    ...
    ##FRENCH GAME TYPE NAMES
    TYPE_NAMES = {
        BAKERS_DOZEN:        n_("Baker's Dozen"),
        BELEAGUERED_CASTLE:  n_("Beleaguered Castle"),
        CANFIELD:            n_("Canfield"),
        FAN_TYPE:            n_("Fan"),
        FORTY_THIEVES:       n_("Forty Thieves"),
        FREECELL:            n_("FreeCell"),
        GOLF:                n_("Golf"),
        GYPSY:               n_("Gypsy"),
        KLONDIKE:            n_("Klondike"),
        MONTANA:             n_("Montana"),
        NAPOLEON:            n_("Napoleon"),
        NUMERICA:            n_("Numerica"),
        PAIRING_TYPE:        n_("Pairing"),
        RAGLAN:              n_("Raglan"),
        SIMPLE_TYPE:         n_("Simple games"),
        SPIDER:              n_("Spider"),
        TERRACE:             n_("Terrace"),
        YUKON:               n_("Yukon"),
        ONE_DECK_TYPE:          n_("One-Deck games"),
        TWO_DECK_TYPE:          n_("Two-Deck games"),
        THREE_DECK_TYPE:          n_("Three-Deck games"),
        FOUR_DECK_TYPE:          n_("Four-Deck games"),
...
##     SELECT_GAME_BY_TYPE = tuple(SELECT_GAME_BY_TYPE)
    ...
    ## ALL FRENCH GAME TYPES
    ####tuple containing (name,function) tuples
    ##lambda methods
    #args = gi,gt
    #expr = gi.si.game_type == gt

    SELECT_GAME_BY_TYPE = (
        (n_("Baker's Dozen type"),lambda gi, gt=BAKERS_DOZEN: gi.si.game_type == gt),
        (n_("Beleaguered Castle type"),lambda gi, gt=BELEAGUERED_CASTLE: gi.si.game_type == gt),
        (n_("Canfield type"), lambda gi, gt=CANFIELD: gi.si.game_type == gt),
        (n_("Fan type"),      lambda gi, gt=FAN_TYPE: gi.si.game_type == gt),
        (n_("Forty Thieves type"),lambda gi, gt=FORTY_THIEVES: gi.si.game_type == gt),
        (n_("FreeCell type"), lambda gi, gt=FREECELL: gi.si.game_type == gt),
        (n_("Golf type"),     lambda gi, gt=GOLF: gi.si.game_type == gt),
        (n_("Gypsy type"),    lambda gi, gt=GYPSY: gi.si.game_type == gt),
        (n_("Klondike type"), lambda gi, gt=KLONDIKE: gi.si.game_type == gt),
        (n_("Montana type"),  lambda gi, gt=MONTANA: gi.si.game_type == gt),
        (n_("Napoleon type"), lambda gi, gt=NAPOLEON: gi.si.game_type == gt),
        (n_("Numerica type"), lambda gi, gt=NUMERICA: gi.si.game_type == gt),
        (n_("Pairing type"),  lambda gi, gt=PAIRING_TYPE: gi.si.game_type == gt),
        (n_("Raglan type"),   lambda gi, gt=RAGLAN: gi.si.game_type == gt),
        (n_("Simple games"),  lambda gi, gt=SIMPLE_TYPE: gi.si.game_type == gt),
        (n_("Spider type"),   lambda gi, gt=SPIDER: gi.si.game_type == gt),
        (n_("Terrace type"),  lambda gi, gt=TERRACE: gi.si.game_type == gt),
        (n_("Yukon type"),    lambda gi, gt=YUKON: gi.si.game_type == gt),
        (n_("One-Deck games"),lambda gi, gt=ONE_DECK_TYPE: gi.si.game_type == gt),
        (n_("Two-Deck games"),lambda gi, gt=TWO_DECK_TYPE: gi.si.game_type == gt),
        (n_("Three-Deck games"),lambda gi, gt=THREE_DECK_TYPE: gi.si.game_type == gt),
        (n_("Four-Deck games"),lambda gi, gt=FOUR_DECK_TYPE: gi.si.game_type == gt),
    )

    SELECT_ORIGINAL_GAME_BY_TYPE = (
        (n_("French type"),         lambda gi, gf=ORIGINAL, gt=(HANAFUDA, HEXADECK, MUGHAL_GANJIFA, NAVAGRAHA_GANJIFA, DASHAVATARA_GANJIFA, TAROCK,): gi.si.game_flags & gf and gi.si.game_type not in gt),
        (n_("Ganjifa type"),        lambda gi, gf=ORIGINAL, gt=(MUGHAL_GANJIFA, NAVAGRAHA_GANJIFA, DASHAVATARA_GANJIFA,): gi.si.game_flags & gf and gi.si.game_type in gt),
        (n_("Hanafuda type"),       lambda gi, gf=ORIGINAL, gt=HANAFUDA: gi.si.game_flags & gf and gi.si.game_type == gt),
        (n_("Hex A Deck type"),     lambda gi, gf=ORIGINAL, gt=HEXADECK: gi.si.game_flags & gf and gi.si.game_type == gt),
        (n_("Tarock type"),         lambda gi, gf=ORIGINAL, gt=TAROCK: gi.si.game_flags & gf and gi.si.game_type == gt),
    )

    SELECT_CONTRIB_GAME_BY_TYPE = (
        (n_("French type"),         lambda gi, gf=CONTRIB, gt=(HANAFUDA, HEXADECK, MUGHAL_GANJIFA, NAVAGRAHA_GANJIFA, DASHAVATARA_GANJIFA, TAROCK,): gi.si.game_flags & gf and gi.si.game_type not in gt),
        (n_("Ganjifa type"),        lambda gi, gf=CONTRIB, gt=(MUGHAL_GANJIFA, NAVAGRAHA_GANJIFA, DASHAVATARA_GANJIFA,): gi.si.game_flags & gf and gi.si.game_type in gt),
        (n_("Hanafuda type"),       lambda gi, gf=CONTRIB, gt=HANAFUDA: gi.si.game_flags & gf and gi.si.game_type == gt),
        (n_("Hex A Deck type"),     lambda gi, gf=CONTRIB, gt=HEXADECK: gi.si.game_flags & gf and gi.si.game_type == gt),
        (n_("Tarock type"),         lambda gi, gf=CONTRIB, gt=TAROCK: gi.si.game_flags & gf and gi.si.game_type == gt),
    )

    ##ORIENTAL GAMES
    SELECT_ORIENTAL_GAME_BY_TYPE = (
        (n_("Dashavatara Ganjifa type"), lambda gi, gt=DASHAVATARA_GANJIFA: gi.si.game_type == gt),
        (n_("Ganjifa type"),        lambda gi, gt=(MUGHAL_GANJIFA, NAVAGRAHA_GANJIFA, DASHAVATARA_GANJIFA,): gi.si.game_type in gt),
        (n_("Hanafuda type"),       lambda gi, gt=HANAFUDA: gi.si.game_type == gt),
        (n_("Mughal Ganjifa type"), lambda gi, gt=MUGHAL_GANJIFA: gi.si.game_type == gt),
        (n_("Navagraha Ganjifa type"), lambda gi, gt=NAVAGRAHA_GANJIFA: gi.si.game_type == gt),
    )

    ##SPECIAL GAMES
    SELECT_SPECIAL_GAME_BY_TYPE = (
        (n_("Shisen-Sho"),      lambda gi, gt=SHISEN_SHO: gi.si.game_type == gt),
        (n_("Hex A Deck type"), lambda gi, gt=HEXADECK: gi.si.game_type == gt),
        (n_("Matrix type"),     lambda gi, gt=MATRIX: gi.si.game_type == gt),
        (n_("Memory type"),     lambda gi, gt=MEMORY: gi.si.game_type == gt),
        (n_("Poker type"),      lambda gi, gt=POKER_TYPE: gi.si.game_type == gt),
        (n_("Puzzle type"),     lambda gi, gt=PUZZLE_TYPE: gi.si.game_type == gt),
        (n_("Tarock type"),     lambda gi, gt=TAROCK: gi.si.game_type == gt),
    )
...
##        156: 5035,              # Mahjongg - Fortress Towers
    ...
    # deprecated - the correct way is to or a GameType.XXX flag
...
class GameInfo(Struct):
    def __init__(self, id, gameclass, name,
        ...
        #
        if not (1 <= category <= 9):
            if game_type == GameType.HANAFUDA:
                category = GI.GC_HANAFUDA
            elif game_type == GameType.TAROCK:
                category = GI.GC_TAROCK
            elif game_type == GameType.MAHJONGG:
                category = GI.GC_MAHJONGG
            elif game_type == GameType.HEXADECK:
                category = GI.GC_HEXADECK
            elif game_type == GameType.MUGHAL_GANJIFA:
                category = GI.GC_MUGHAL_GANJIFA
            elif game_type == GameType.NAVAGRAHA_GANJIFA:
                category = GI.GC_NAVAGRAHA_GANJIFA
            elif game_type == GameType.DASHAVATARA_GANJIFA:
        ...
        #
        for f, l in ((GameType.CHILDREN, GI._CHILDREN_GAMES),
                     (GameType.OPEN, GI._OPEN_GAMES),
                     (GameType.POPULAR, GI._POPULAR_GAMES)):
...
class GameManager:
    ...
    def register(self, gi):
        ...
        if self.check_game:
            self._check_game(gi)
        ##if 0 and gi.si.game_flags & GameType.XORIGINAL:
        ##    return
        ##print gi.id, gi.name
        ...
        if not (gi.si.game_flags & GameType.HIDDEN):
...
##             if not gi.si.game_type == GameType.MAHJONGG:
