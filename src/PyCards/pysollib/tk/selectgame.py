...
#from pysollib.gametype import STYLE, SKILL, FR_TYPES, FILTER
...
class SelectGameData(SelectDialogTreeData):
    def __init__(self, app):
        ...
        g = []
        for data in (FILTER.FRENCH, #gametype.SELECT_FR_GAME_BY_TYPE,
                     FILTER.ORIENTAL,
                     FILTER.SPECIAL,
                     FILTER.ORIgametypeNAL,
                     FILTER.CONTRIB,
                     ):
            gg = []
            ...
            g.append(gg)
        select_mahjongg_game = lambda gi: gi.si.game_type == STYLE.MAHJONGG
        gg = None
...
##             s_contrib = SelectGameNode(None, "Contributed Games", tuple(g[4]))
        ...
        for name, games in GAMES_BY_COMPATIBILITY:
        ...
        for name, games in GAMES_BY_PYSOL_VERSION:
            select_func = lambda gi, games=games: gi.id in games
            if name is None or not filter(select_func, self.all_games_gi):
               continue
        ...
        for name, games in GAMES_BY_INVENTORS:
            select_func = lambda gi, games=games: gi.id in games
            if name is None or not filter(select_func, self.all_games_gi):
               continue
            gg.append(SelectGameNode(None, name, select_func))
        ...
        self.rootnodes = filter(None, (
            ...
            SelectGameNode(None, _("Popular Games"),
                           lambda gi: gi.si.game_flags & STYLE.POPULAR),
            s_mahjongg,
            s_oriental,
            s_special,
            SelectGameNode(None, _("Custom Games"),
                           lambda gi: gi.si.game_type == STYLE.CUSTOM),
            s_by_type,
            ...
            SelectGameNode(None, _("Other Categories"), (
                SelectGameNode(None, _("Games for Children (very easy)"),
                               lambda gi: gi.si.game_flags & STYLE.CHILDREN),
                SelectGameNode(None, _("Games with Scoring"),
                               lambda gi: gi.si.game_flags & STYLE.SCORE),
                SelectGameNode(None, _("Games with Separate Decks"),
                               lambda gi: gi.si.game_flags & STYLE.SEPARATE_DECKS),
                SelectGameNode(None, _("Open Games (all cards visible)"),
                               lambda gi: gi.si.game_flags & STYLE.OPEN),
                SelectGameNode(None, _("Relaxed Variants"),
                               lambda gi: gi.si.game_flags & STYLE.RELAXED),
            )),
            s_original,
            s_contrib,
        ))
...
class SelectGameDialogWithPreview(SelectGameDialog):
    ...
    def updatePreview(self, gameid, animations=10):
        ...
        self.random.origin = self.random.ORIgametypeN_PREVIEW # FIXME pretty sure this should be ORIGIN
    ...
    def updateInfo(self, gameid):
        ...
        type = ''
        if gi.si.game_type in FR_TYPES:
            type = _(FR_TYPES[gi.si.game_type])
        sl = {
