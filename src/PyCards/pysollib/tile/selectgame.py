...
class SelectGameData(SelectDialogTreeData):
    def __init__(self, app):
        ...
        self.all_games_gi = map(app.gdb.get, app.gdb.gamesNameSorted())
        ...
        for data in (FILTER.FRENCH,
            ...
            g.append(gg)
        select_mahjongg_game = lambda gi: gi.game_type == STYLE.MAHJONGG
        gg = None
...
##             s_contrib = SelectGameNode(None, "Contributed Games", tuple(g[4]))
        ...
        for name, games in gametype.GAMES_BY_COMPATIBILITY:
        ...
        for name, games in gametype.GAMES_BY_PYSOL_VERSION:
            select_func = lambda gi, games=games: gi.ID in games
            if name is None or not filter(select_func, self.all_games_gi):
               continue
        ...
        for name, games in gametype.GAMES_BY_INVENTORS:
            select_func = lambda gi, games=games: gi.ID in games
            if name is None or not filter(select_func, self.all_games_gi):
               continue
            gg.append(SelectGameNode(None, name, select_func))
        if 1 and gg:
            s_by_inventors = SelectGameNode(None, _("by Inventors"),
                                            tuple(gg))
        #
        ul_alternate_names = UserList(list(app.gdb.gamesAltNameSorted()))
        #
...
##                                    lambda gi: gi.redeals == -2),
            ...
            SelectGameNode(None, _("Other Categories"), (
                SelectGameNode(None, _("Games for Children (very easy)"),
                               lambda gi: gi.game_flags & STYLE.CHILDREN),
                SelectGameNode(None, _("Games with Scoring"),
                               lambda gi: gi.game_flags & STYLE.SCORE),
                SelectGameNode(None, _("Games with Separate Decks"),
                               lambda gi: gi.game_flags & STYLE.SEPARATE_DECKS),
                SelectGameNode(None, _("Open Games (all cards visible)"),
                               lambda gi: gi.game_flags & STYLE.OPEN),
                SelectGameNode(None, _("Relaxed Variants"),
                               lambda gi: gi.game_flags & STYLE.RELAXED),
            )),
            s_original,
            s_contrib,
        ))
...
class SelectGameDialogWithPreview(SelectGameDialog):
    ...
    def updateInfo(self, gameid):
        ...
        type = ''
        if gi.game_type in gametype.FR_TYPES:
            type = _(gametype.FR_TYPES[gi.game_type])
        sl = {
            SKILL.LUCK:         _('Luck only'),
            SKILL.MOSTLY_LUCK:  _('Mostly luck'),
            SKILL.BALANCED:     _('Balanced'),
            SKILL.MOSTLY_SKILL: _('Mostly skill'),
            SKILL.SKILL:        _('Skill only'),
            }
